...
% image. Original code, uses first five principle components.
%
%	INPUT
...
%       debug: BOOL whether to use under debug mode
...
default_values = {0,0,0,'click','pca',4,0,'muct'};
...
for n_resolution = 1:n_resolutions
    ...
    n_pcs = 5; %takes the first five principle components
    ...
    if n_resolution == 1
        if strcmpi(face_find_method,'auto')
            ...
            [x_original_estimate] = placeShape(im,x_mean,layout);
        end, tic, drawnow
    ...
    for n_evolution = 1:n_evolutions;
            ...
            for c = -(search_size/2):(search_size/2)
                for r = -(search_size/2):(search_size/2)
                    ...
                    if strcmpi(dist_metric,'pca')
                        ...
                        g_P = g_eVecs{n_landmark}(:,1:n_pcs); 
                        eVals = g_eVals{n_landmark}(1:n_pcs);
        ...
        if vis % View the current evolution            
            ...
            plot(x_suggested(1:2:end),x_suggested(2:2:end),'bo','linewidth',4), drawnow() %bo is blue circle
...
if debugg
    ...
    for n = 1:(size(x_final, 1) / 2)
        hold on,
        index = 2 * n;
        landmark_x = x_final(index - 1);
        landmark_y = x_final(index);

        plot(landmark_x, landmark_y, 'rs', 'markerfacecolor', 'r', 'MarkerSize', 5);
        text(landmark_x, landmark_y, sprintf('%d', n), 'color', 'g', 'fontsize', 7);
    end
    
    % Hardcode draw lines to for each key landmark
    landmarks_chest_x = [x_final(1) x_final(3)];
    landmarks_chest_y = [x_final(2) x_final(4)];
    
    landmarks_shoulder_x = [x_final(5) x_final(7) x_final(9) x_final(11) x_final(13)];
    landmarks_shoulder_y = [x_final(6) x_final(8) x_final(10) x_final(12) x_final(14)];
    
    landmarks_arm_size_x = [x_final(15) x_final(17)];
    landmarks_arm_size_y = [x_final(16) x_final(18)];
    
    landmarks_waist_x = [x_final(19) x_final(21)];
    landmarks_waist_y = [x_final(20) x_final(22)];
    
    landmarks_arm_length_x = [x_final(23) x_final(25)];
    landmarks_arm_length_y = [x_final(24) x_final(26)];
    
    hold on,
    plot(landmarks_chest_x, landmarks_chest_y, 'r');
    plot(landmarks_shoulder_x, landmarks_shoulder_y, 'r');
    plot(landmarks_arm_size_x, landmarks_arm_size_y, 'r');
    plot(landmarks_waist_x, landmarks_waist_y, 'r');
    plot(landmarks_arm_length_x, landmarks_arm_length_y, 'r');
